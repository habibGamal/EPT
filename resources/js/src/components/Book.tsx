    ...
    return (
        <div className="rounded bg-stone-100 shadow-sm p-4">
            ...
            <form action={book ? `/api/book_update/${book.id}` : "/api/save-book"} method='post' encType="multipart/form-data">
                ...
                {
                    book ?
                        <form className='inline-block mx-5' action={`/api/book_delete/${book.id}`} method='post'><button className="rounded bg-red-600 text-white shadow font-sans p-2">delete</button></form> : ''
                }
            </form>
        </div>
    )
